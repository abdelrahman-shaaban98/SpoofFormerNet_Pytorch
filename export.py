...
def _export_torchscript(model, output_path, rgb_dummy, depth_dummy):
...
def _export_onnx(model, output_path, rgb_dummy, depth_dummy):
    Path(output_path).parent.mkdir(parents=True, exist_ok=True)
...
def export(
    ...
    if export_to == "torchscript":
        ...
        _export_torchscript(model, output_path, rgb_dummy, depth_dummy)
    
    elif export_to == "onnx":
        print(f"\n  Method       : onnx") 
        output_path = output_path + ".onnx"
        _export_onnx(model, output_path, rgb_dummy, depth_dummy)
